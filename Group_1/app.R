...
#              img(src = "X-1.jpg", width = 700),
      ...
      tabItem(tabName = "pg1",
              ...
              br(),
              column(width = 4,
                     sliderInput("year1", "Year:", min = 2005, max = 2018, value = 2000, step = 1, 
      ...
      tabItem(tabName = "pg2",
              ...
              br(),
              column(width = 4,
                     sliderInput("bedrm", "Bedroom Number:", min = 1, max = 7, value = 4, step = 1, 
                                 animate = animationOptions(interval = 1000, loop = TRUE)),
                     br(),
                     sliderInput("bathrm", "Bathroom Number:", min = 1, max = 5, value = 3, step = 1, 
                                 animate = animationOptions(interval = 1000, loop = TRUE)),
                     br(),
      ...
      tabItem(tabName = "pg4",
              ...
              br(),
              column(width = 2,
              radioButtons("radio2", "Location:", choices = list("NW" = "NW", "SW" = "SW",
                                                                "NE" = "NE", "SE" = "SE"))
              ),
      ...
      tabItem(tabName = "map",
              ...
              column(width = 4, 
                     ...
                     radioButtons("radio", "Location:", choices = list("NW" = "NW", "SW" = "SW",
                                                                       "NE" = "NE", "SE" = "SE"))
                     ),
...
server <- function(input, output) {
  ...
  subdata1 <- reactive({data[data$YEAR==input$year1, ]})
  output$plot1 <- renderPlot({
    ggplot(data = subdata1(), aes(PRICE)) + geom_histogram(binwidth = 100000, fill = "steelblue") + xlim(0,2500000) + ylim(0,500) + 
  ...
  subdata2 <- reactive({data[data$BEDRM==input$bedrm & data$BATHRM==input$bathrm & data$AC==input$ac, ]})
  output$plot2 <- renderPlot({
    ggplot(data = subdata2(), aes(x=YEAR, y=PRICE)) + geom_point() + ylim(0,2000000) + 
  ...
  subdata4 <- reactive({data[data$QUADRANT==input$radio2,]})
  output$plot4 <- renderLeaflet({
    leaflet(data = subdata4()) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addWebGLHeatmap (lng = ~subdata4()$LONGITUDE, lat = ~subdata4()$LATITUDE,
...
# AC (Y/N): ac2
  ...
  subdata5 <- reactive({data[data$QUADRANT==input$radio & data$PRICE<=input$range[2] & data$PRICE>=input$range[1] 
  ...
  output$plot5 <- renderLeaflet({
    leaflet(data = subdata5()) %>%
      addTiles() %>%
      addMarkers(lng = ~subdata5()$LONGITUDE, lat = ~subdata5()$LATITUDE, 
                 popup = ~paste0("Price: $",subdata5()$PRICE, br(),
